...
FK_SQL = "INTEGER" if IS_SQLITE else "BIGINT"
TIMESTAMP_SQL = "TEXT" if IS_SQLITE else "TIMESTAMPTZ"
    ...
    def fetchone(self):
        if self._buffered_rows is not None:
            if not self._buffered_rows:
                return None
            return _normalize_record(self._buffered_rows[0])
        row = self._result.mappings().first()
        return _normalize_record(row) if row else None

    def fetchall(self):
        if self._buffered_rows is not None:
            return [_normalize_record(row) for row in self._buffered_rows]
        return [_normalize_record(row) for row in self._result.mappings().all()]
    ...
    with db_session() as conn:
        ...
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS users (
                ...
                is_active INTEGER DEFAULT 1,
                created_at {TIMESTAMP_SQL} NOT NULL DEFAULT {NOW_SQL},
                last_login {TIMESTAMP_SQL},
                failed_attempts INTEGER DEFAULT 0,
                locked_until {TIMESTAMP_SQL},
        ...
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS messages (
                id {PRIMARY_KEY_SQL},
                sender_id {FK_SQL} NOT NULL,
                receiver_id {FK_SQL} NOT NULL,
        ...
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS wa_contacts (
                ...
                first_seen_at {TIMESTAMP_SQL} NOT NULL DEFAULT {NOW_SQL},
                last_message_at {TIMESTAMP_SQL},
        ...
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS wa_transfer_log (
                ...
                from_user_id {FK_SQL},
                to_user_id {FK_SQL},
                from_department_id {FK_SQL},
                to_department_id {FK_SQL},
                reason TEXT DEFAULT '',
                summary TEXT DEFAULT '',
                transferred_by {FK_SQL} NOT NULL,
                created_at {TIMESTAMP_SQL} NOT NULL DEFAULT {NOW_SQL},
                FOREIGN KEY (contact_id) REFERENCES wa_contacts(id),
        ...
        _migrate(conn, "wa_contacts", "assigned_to", FK_SQL)
        _migrate(conn, "wa_contacts", "department_id", FK_SQL)
        ...
        _migrate(conn, "users", "department_id", FK_SQL)
        _migrate(conn, "users", "avatar_path", "TEXT DEFAULT ''")
        _migrate(conn, "users", "role", "TEXT DEFAULT 'operador'")
        _migrate(conn, "users", "email", "TEXT DEFAULT ''")
        _migrate(conn, "users", "firebase_uid", "TEXT DEFAULT ''")
        _migrate(conn, "users", "auth_provider", "TEXT DEFAULT 'legacy'")
...
def get_user_by_id(user_id):
    with db_session() as conn:
        row = conn.execute(
            "SELECT id, username, display_name, department_id, role, is_active, email, firebase_uid, auth_provider, "
    ...
    with db_session() as conn:
        rows = conn.execute(
            """SELECT u.id, u.username, u.display_name, u.is_active, u.last_login,
                      u.department_id, u.avatar_path, u.role, u.email, u.firebase_uid, u.auth_provider,
...
def create_user(username, display_name, password_hash, department_id=None, role="operador"):
    with db_session() as conn:
        ...
        if existing:
            return None
        cursor = conn.execute(
            "INSERT INTO users (username, display_name, password_hash, department_id, role, email, firebase_uid, auth_provider) "
            "VALUES (?, ?, ?, ?, ?, '', '', 'legacy')",
...
def sync_user_identity(user_id, email=None, firebase_uid=None, auth_provider="firebase", display_name=None):
    with db_session() as conn:
        ...
        values = [auth_provider or "firebase"]
...
def upsert_firebase_user(firebase_uid, email, display_name="", role=None, department_id=None):
    ...
    base_username = (email or firebase_uid or "firebase_user")[:50]
